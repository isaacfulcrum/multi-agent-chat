...
export class ChatService {
  ...
  * @param onComplete an optional callback that will be called when the completion is done
  */
  public async runCompletion(messages: ChatCompletionRequestMessage[], agent?: Agent, onComplete?: () => void) {
    try {
      ...
      const id = nanoid(); // can the id be generated by the API?
      let chatMessage: AssistantChatMessage = { id, role: ChatMessageRole.Assistant, content: "", isAgent: false };

      /* If an agent is passed, we add its description as a system message.
